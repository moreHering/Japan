-- 0015 · Kurzlink-Auflöser: Diagnose wieder schlank
--
-- Befund aus 0014 (26.09.2026, echter Link einer Unterkunft in Osaka): Die
-- Kartenseite, auf die der Kurzlink führt, enthält **keine** Koordinate. Google
-- lädt den Ort erst nachträglich per JavaScript über eine undokumentierte
-- Schnittstelle — darauf baut diese App nicht. Was der Server verlässlich
-- liefert, ist das Weiterleitungsziel mit der **Adresse**; die sucht der Client
-- dann bei OpenStreetMap (src/lib/kurzlink.ts, `adressVarianten`).
--
-- Die Suche nach Zahlenpaaren über 800 kB Seitentext kostete bei jedem Aufruf
-- Zeit und brachte nichts; sie fällt weg. Status und Titel bleiben als Auskunft.
-- Links, deren Ziel `!3d…!4d…` oder `@lat,lng` trägt, lösen weiter direkt auf.
